document.addEventListener("DOMContentLoaded", function () {
  ...
  function pilihVideoUntukPoin(magnitude, positif) {
    if (positif) {
      if (magnitude >= 5000) {
        return "tinggi1.mp4";
      }
  ...
  function generateAura() {
    ...
    return {
      ...
      videoFile: pilihVideoUntukPoin(magnitude, positif),
    };
  }
  ...
  function tampilkanHasil(nama, hasil) {
    ...
    if (!hasil.glitch) {
      video.src = `Meme/${hasil.videoFile}`;
      video.autoplay = true;
      video.muted = false;
      video.loop = true;
      ...
      video.play().catch(() => {
        console.log(
          "Audio/video tidak bisa autoplay, tetapi file sudah siap diputar.",
        );
      });
    } else {
